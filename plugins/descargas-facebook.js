...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  if (!args[0]) return m.reply('☁️ Ingresa un enlace de facebook');

  try {
await m.react('🕓'); 
    const apiResponse = await fetch(`https://apis-starlights-team.koyeb.app/starlight/facebook?url=${args[0]}`);
